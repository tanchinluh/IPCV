function out = bwmorph(image, operation, iterations)
    // Dispatch common binary morphology operations through IPCV primitives.
    rhs = argn(2);
    if rhs < 2 then error("bwmorph requires an image and operation."); end
    ...
    mask = ipcv_binary_mask(image, "bwmorph");
    operation = convstr(operation, "l");
    se = imcreatese("cross", 3, 3);
    select operation
    case "thin" then out = bwthin(mask, "zhang-suen");
    case "skel" then out = bwthin(mask, "zhang-suen");
    case "fill" then out = bwfill(mask);
    ...
    case "hitmiss" then out = bwhitmiss(mask, se, iterations);
