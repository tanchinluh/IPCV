function bwout = imbwareafilt(image, areaRange, connectivity)
    // Keep connected foreground components within an inclusive area range.
    rhs = argn(2);
    if rhs < 2 then error("imbwareafilt requires an image and an area range."); end
    ...
    mask = ipcv_binary_mask(image, "imbwareafilt");
