function out = imadjust3(volume, limits)
    ...
    if typeof(volume(1)) == "constant" then values = double(volume); else values = im2double(volume); end; if size(size(values), "*") <> 3 then error("imadjust3: a 3-D volume is required."); end
