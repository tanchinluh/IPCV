...
int sci_int_imgaussianblur(char *fname, void *pvApiCtx)
{
    ...
    iRet = get_scalar_double_arg(pvApiCtx, 4, &sigmaX);
    if (iRet)
    {
        ipcv_release_image_argument(source);
        return iRet;
    }
    iRet = get_scalar_double_arg(pvApiCtx, 5, &sigmaY);
    if (iRet)
    {
        ipcv_release_image_argument(source);
        return iRet;
    ...
    iRet = ipcv_gaussian_blur_image(&source, static_cast<int>(floor(kernelRows + 0.5)), static_cast<int>(floor(kernelCols + 0.5)), sigmaX, sigmaY, &output);
    ipcv_release_image_argument(source);
    if (iRet)
    {
        Scierror(999, "%s: %s\n", fname, output.error);
