...
int sci_avilistopened(char * fname,void* pvApiCtx)
{
	...
	{
		if(OpenedAvi[i].cap.isOpened() | OpenedAvi[i].writer.isOpened())
		{
			dIndices[count]=i+1;

			//strcpy(sFileNames[count],  OpenedAviCap[i].filename, MAX_FILENAME_LENGTH);
			strncpy(sFileNames+offset, OpenedAvi[i].filename, MAX_FILENAME_LENGTH);
			offset += (int)strlen(OpenedAvi[i].filename)+1;
			count++;
		}
	}
