...
int sci_int_getaffinetransform(char *fname, void *pvApiCtx)
{
    ...
    if (iRet)
    {
        Scierror(999, "%s: Wrong type for input argument #%d: 3x2 source point matrix expected.\n", fname, 1);
        return iRet;
    }
    ...
    if (iRet)
    {
        Scierror(999, "%s: Wrong type for input argument #%d: 3x2 target point matrix expected.\n", fname, 2);
        ipcv_release_image_argument(sourcePoints);
        return iRet;
    }

    iRet = ipcv_get_affine_transform_matrix(&sourcePoints, &targetPoints, &output);
    ipcv_release_image_argument(sourcePoints);
