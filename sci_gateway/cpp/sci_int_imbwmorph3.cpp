...
extern "C" int sci_int_imbwmorph3(char *fname, void *pvApiCtx)
{
    ...
    for (int position = 2; !result && position <= 4; ++position)
    {
        int *address = NULL;
    ...
    if (!result) result = ipcv_binary_morphology_volume(&source,
        static_cast<int>(std::round(arguments[0])),
        static_cast<int>(std::round(arguments[1])),
        static_cast<int>(std::round(arguments[2])), &output);
