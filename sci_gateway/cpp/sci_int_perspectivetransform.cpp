...
int sci_int_perspectivetransform(char *fname, void *pvApiCtx)
{
    ...
    if (iRet)
    {
        Scierror(999, "%s: Wrong type for input argument #%d: 3x3 perspective matrix expected.\n", fname, 2);
    ...
    iRet = ipcv_perspective_transform_image(&source, &matrix, static_cast<int>(*heightValue), static_cast<int>(*widthValue), &output);
    ipcv_release_image_argument(source);
