function IPCV_MacOS(root_tlbx,OPENCV_LIBS)
    ...
    if (is_x64) then
        opencvDynLibPath = fullpath(root_tlbx + "/thirdparty/opencv/" + "MacOS"  + "/x64/lib/");
    else
        opencvDynLibPath = fullpath(root_tlbx + "/thirdparty/opencv/" + "MacOS" + "/x86/lib/");
    end
    pp = pwd();
    cd(opencvDynLibPath);
    while 1
